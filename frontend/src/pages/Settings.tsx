    ...
    <>
                          ...
                          <SelectContent position="popper">
                            <SelectGroup>
                              {enums.roles?.map((role) => (
                                <SelectItem id={role.value} value={role.value}>
                                  {role.label}
                                </SelectItem>
                              ))}
                            </SelectGroup>
              ...
              <div className="w-2/3">
                <Select value={selectedUserId} onValueChange={(value) => setSelectedUserId(value)}>
                  ...
                  <SelectContent position="popper">
                    <SelectGroup>
                      {userList.map((user) => (
                        <SelectItem id={`${user.id}`} value={`${user.id}`}>
                ...
                <div className="text-slate-400 mt-4 grid grid-cols-2 grid-rows-2 gap-4">
                  ...
                  <Field>
                    ...
                    <Input
                      disabled
                      value={
                        watchedUser.created_at
                          ? dayjs.utc(watchedUser.created_at).local().format('DD.MM.YYYY HH:mm')
                          : ''
                      }
                    />
                  </Field>
                  <Field>
                    ...
                    <Input
                      disabled
                      value={
                        watchedUser.updated_at
                          ? dayjs.utc(watchedUser.updated_at).local().format('DD.MM.YYYY HH:mm')
                          : ''
                      }
                    />
                  </Field>
                ...
                <form onSubmit={editUserForm.handleSubmit(submitEditUser)}>
                  ...
                  <div className="mt-4 flex gap-2">
                    <Button
                      variant="secondary"
                      disabled={!selectedUserId || editUserLoading}
                      type="submit">
      ...
      <div className="mt-5 text-slate-100 w-full bg-[#0c1327] p-5 border-gray-800 border rounded-xl">
        ...
        <div className="mx-20 flex flex-col gap-2 text-slate-400">
          ...
          <p className="text-sm">
            Последнее изменение:{' '}
            {dayjs.utc(user.userData?.password_updated).local().format('DD MMMM YYYY, HH:mm')}
          </p>
          {changePasswordState && (
            <form
              className="w-1/2 flex flex-col gap-2"
              onSubmit={changePasswordForm.handleSubmit(submitChangePassword)}>
